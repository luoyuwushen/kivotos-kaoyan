...
登录页按本地时间显示阿洛娜的日／夜教室，使用 Spine 实时绘制；密码登录成功后每次都会播放进入画面，完成再进入应用。画面无法加载时，静态教室预览仍会显示，登录操作可继续。实现与验收见 [登录场景移植说明](docs/shittim-port.md)。
...
> 站内 Q 版形象是**本项目自绘的原创图形**；**登录场景**使用的是官方 / 同人的角色与场景素材
> （Spine 导出数据，来源见 [`docs/shittim-port.md`](docs/shittim-port.md)），仅作个人非商业用途，
> 本站不主张任何权利，为非官方粉丝作品。
> **如有侵权，请联系 2651038380@qq.com，我们会立即移除。**
> 登录场景由 Spine 运行时实时渲染（© Esoteric Software LLC，依 Spine Runtimes License Agreement）。
...
配好后端之后，站点打开就是一个**独立的登录屏**：左边是初试倒计时与四个阶段的进度条，
右边是一块「事务局窗口」式的表单。没登录之前进不去应用，地址栏会被收敛到 `#login`
（`#signup` / `#forgot` 分别是注册与找回密码，可以直接收藏）。
...
|----------|--------|
| 用密码登录 | 邮箱 + 密码 → **登录** |
| 注册 | 登录屏 → **还没有账号？去注册**（要输两次密码，不一致会在字段下面提示） |
| 忘了密码 | 登录屏 → **忘记密码** → 收邮件 → 点回来 → **设置新密码** → 用新密码登录 |
| 不想记密码 | 填好邮箱 → **发一封登录链接**，点邮件里的链接直接登录 |

> 密码框右边有个眼睛图标，可以临时把密码显示成明文 —— 在手机上确认有没有输错很有用。
...
- 「退出登录」（设置 → 云端同步）会把你送回登录屏，本机数据一条都不会动；
  如果登录状态在别处失效了（换设备、token 过期），下一次操作也会自动退回登录屏
...
| 忘记密码了 | 登录屏 → **忘记密码** → 收重置邮件 → 点回来直接设新密码（不用输旧密码） |
| 重置链接提示「已经失效」 | 链接只能用一次。回登录屏再点一次「忘记密码」重发一封即可 |
| 一直卡在登录屏，登录没反应 | 打开控制台看有没有报错；多半是 Redirect URLs 没配（见下一节第 3 条） |
...
| 3 | URL Configuration → **Redirect URLs** | 加一条本站地址，例如 `https://luoyuwushen.github.io/kivotos-kaoyan/**` | 魔法链接 / 重置密码链接点回来会跳错地方，登不上 |

> 第 3 条同时管「登录链接」和「重置密码链接」—— 两者都是把 token 带回本站兑换，
> 地址不在白名单里就会被 Supabase 拒掉。填 `/**` 结尾可以一次覆盖所有页面。
