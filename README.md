...
**靠浏览器隔离，不靠账号**：同一个网址，你用手机打开是你自己的数据，同学用他的手机打开
是一份全新的空数据，互相看不到。因为根本没有服务器，你的记录从未离开过你的设备。

**要不要加后端？** 详细分析与可直接复制的代码见 **[`docs/后端与多用户方案.md`](docs/后端与多用户方案.md)**，
里面写了三条路径（本地导出 / GitHub Gist 同步 / Supabase 多用户）、
「如何识别不同用户」的实现原理（JWT + 行级安全 RLS），以及为什么推荐 Supabase 而不是自己写服务。
