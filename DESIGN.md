...
左栏是这场作战本身（品牌 + 倒计时 + 四阶段时间轴），右栏是一块**事务局窗口**——
...
现在唯一压在裸天色上的只剩那串巨型倒计时数字 —— 它是大字，门槛 3:1。
...
| 阶段名 | 11.22:1 / 16.26:1 |
| 表单链接 | 5.93:1 / 6.26:1 |
| 倒计时巨字（大字，门槛 3:1） | 6.5:1 上下 |

审计脚本：`scripts/verify-login.mjs`（第 7 节，共 18 项，浅色深色各 9 项）。

> **审计本身也有两个坑**，都写进脚本注释了：
> ① `page.screenshot()` 对「看起来没变」的页面会复用上一次的截图，纯改样式的两次审计
> 会量到同一张图 —— 必须走 CDP `Page.captureScreenshot`；
