...
- [x] 阿洛娜 / 普拉娜 挂件
- [x] 登录页实时绘制日／夜教室与角色；每次密码登录成功后播进入画面，再进入应用。素材失败可用静态图继续登录
...
快捷键与命令面板、JSON 备份与恢复、三端适配、日／夜登录场景与进入动画、GitHub Pages 自动部署、
...
- ❌ 不热链第三方图片（素材一律自托管；见 DESIGN.md「素材与版权口径」）。
      登录场景的官方 / 同人 Spine 素材也随站点自托管，仅作个人非商业用途；
      来源和声明见 `docs/shittim-port.md`。

## 技术栈
- **构建**：Vite（开发热更新 / 生产打包成静态文件）
- **运行时**：原生 JavaScript ES Modules + 原生 CSS（无 React/Vue，降低小白的理解与维护成本）
- **依赖**：主界面按需加载 `@supabase/supabase-js`；登录场景按需加载 Spine 4.2 官方运行时
